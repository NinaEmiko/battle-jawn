const AccountSettings = ({props}:{props:any}) => {

  return (
    <div className="container-jawn-login-form">
        <h1 className="title-jawn">Account Settings</h1>
            <p className="account-settings-jawn">Edit Username</p>
            <p className="account-settings-jawn">Edit Password</p>
            <p className="account-settings-jawn">Unsubscribe from Mailing List</p>
            <p className="account-settings-jawn">Delete Account</p>
    </div>
  );
};

export default AccountSettings;
